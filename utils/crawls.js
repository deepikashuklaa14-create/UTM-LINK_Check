...
//   try {
//     const u = new URL(raw);
//     const cleanPath = u.pathname.replace(/\/+$/, ''); // remove trailing slash
...
// // Checks if URL has duplicate UTM params
...
//   // For array fields like missingUTM, convert to string for Excel
//   if (Array.isArray(row.missingUTM)) {
//     row.missingUTM = row.missingUTM.length ? row.missingUTM.join(', ') : '';
//   }

//   // For boolean fields, convert to Yes/No or true/false string
//   if (typeof row.duplicateUTM === 'boolean') {
//     row.duplicateUTM = row.duplicateUTM ? 'Yes' : 'No';
//   }
//   if (typeof row.isBrokenLink === 'boolean') {
//     row.isBrokenLink = row.isBrokenLink ? 'Yes' : 'No';
//   }
...
//     });
//   } catch (err) {
//     console.warn('⚠️ Skipping target removal due to navigation issue:', err.message);
//   }
...
//   // Push only if not already visited for utm mode
...
//     if (options.mode === 'utm') {
//       // Only process internal links recursively
...
//         await _crawl(link, options);
//       } else {
//         // For external links in broken or links mode
